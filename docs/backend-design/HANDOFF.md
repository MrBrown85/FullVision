...
- `2026-04-30 | session-27 | persistence-phase3 | Fixed 3 localStorage-only data-loss bugs: saveModules (stash), saveNotes (_persistNotesToCanonical + _noteIds session registry, delete-then-reinsert on edit), saveCustomTags (_persistCustomTagsToCanonical, create-only — no delete RPC). Added 11 e2e-real persistence specs (Phase 3): goals, modules, notes, observations, rubrics, reflections, term-ratings, tag-scores, custom-tags, course-config, section-overrides. Total suite: 80 tests. PR #104 open.`

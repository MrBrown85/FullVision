...
-- ── §2.2a Course-wide student profile summaries ────────────────────────────
-- Bulk companion to get_student_profile() for roster/report surfaces.
-- Returns one row per active enrollment with summary-only student-record data.
create or replace function list_course_student_profiles(p_course_id uuid)
returns table(
    student jsonb,
    enrollment jsonb,
...
--    per call, and get_gradebook / list_course_student_profiles invoke it
--    indirectly per-student. A class
